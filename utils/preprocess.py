...
def read_h5(filename):
    adata = pd.read_hdf(filename, key='data')
    X = np.array(adata.values)
    cell_type, cell_label = np.unique(adata.index, return_inverse=True)
    return X, cell_label
...
def read_h5ad(filename):
    ...
    cell_type, cell_label = np.unique(cell_name, return_inverse=True)
    print("cell type no.=", cell_label.max()+1)
    return X, cell_label
